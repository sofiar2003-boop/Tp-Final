...
import { CheckCircle2, Circle, Calendar, ChevronLeft, ChevronRight, Plus, Trash2, AlertCircle } from 'lucide-react';

export default function HabitManager() {
    // 💾 Carga inicial de hábitos
    const [habits, setHabits] = useState(() => {
        const saved = localStorage.getItem('panel_habitos_mensual_v3');
        return saved ? JSON.parse(saved) : [
            { id: 1, nombre: "Estudiar Electrónica", diasSemana: [1, 2, 3, 4, 5], historialCompletados: [] },
            { id: 2, nombre: "Beber 2L de agua", diasSemana: [0, 1, 2, 3, 4, 5, 6], historialCompletados: [] }
        ];
    });
    ...
    // 🌟 Evitamos que Chrome traduzca rompiendo el string para 'Viernes' y 'Sábado'
    const diasDeLaSemanaNombre = ['Dom', 'Lun', 'Mar', 'Mié', 'Jue', 'Vi\u200Be', 'Sá\u200Bb'];

    // 🎯 FECHAS CLAVES (Cargá acá tus parciales, entregas de laboratorios, etc.)
    // Formato: YYYY-MM-DD
    const fechasClaves = {
    ...
    useEffect(() => {
        localStorage.setItem('panel_habitos_mensual_v3', JSON.stringify(habits));
    }, [habits]);
    ...
    // Verificar si el día seleccionado tiene un evento clave
    const eventoClaveDeHoy = fechasClaves[fechaSeleccionadaStr];

    return (
        <div className="space-y-6 transition-colors duration-300">
            
            {/* 🏛️ SECCIÓN 1: FORMULARIO DE CREACIÓN */}
            ...
            {/* 📅 SECCIÓN 2: VISTA DIARIA DINÁMICA */}
            <div className="bg-white dark:bg-dark-card p-6 rounded-3xl border border-sage-200/60 dark:border-dark-border shadow-2xs">
                <div className="flex items-center gap-6 mb-6 pb-4 border-b border-slate-100 dark:border-dark-border">
                    <div className="flex items-center gap-2 shrink-0">
                        <button 
                            type="button"
                            onClick={() => setFechaSeleccionada(prev => {
                                const nueva = new Date(prev.getTime());
                                nueva.setDate(nueva.getDate() - 1);
                                return nueva;
                            })}
                            className="p-2 bg-slate-50 dark:bg-dark-bg rounded-xl border border-slate-200/60 dark:border-dark-border hover:bg-slate-100 dark:hover:bg-dark-card cursor-pointer"
                        >
                    ...
                    <div className="flex-1 text-center">
                        <h3 className="font-serif font-bold text-lg text-sage-900 dark:text-dark-text-base capitalize">
                            {obtenerEtiquetaDiaDinamica(fechaSeleccionada)}
                        </h3>
                        <p className="text-[11px] text-slate-400 dark:text-dark-text-muted mt-0.5">
                    ...
                    <button 
                        type="button"
                        onClick={() => { setFechaSeleccionada(new Date()); setMesVisualizado(new Date()); }}
                        className="px-4 py-2 bg-sage-50 text-sage-700 dark:bg-dark-bg dark:text-dark-text-base text-xs font-bold rounded-xl border border-sage-100 dark:border-dark-border cursor-pointer hover:bg-sage-100 transition-all shrink-0"
                    >
                        Volver a Hoy
                    </button>
                </div>

                {/* 🌟 INDICADOR DE FECHA CLAVE / EVENTO */}
                {eventoClaveDeHoy && (
                        ...
                        habitoDelDiaFiltrados.map(h => {
                            ...
                            return (
                                <div key={h.id} className="flex items-center justify-between p-3.5 bg-sage-50/40 dark:bg-dark-bg/40 rounded-xl border border-slate-100/80 dark:border-dark-border/40 hover:bg-white dark:hover:bg-dark-bg transition-all group">
                                    <button 
                                        onClick={() => toggleHabitoFecha(h.id, fechaSeleccionadaStr)}
                                        className="flex items-center gap-3 text-left flex-1 cursor-pointer"
                                    >
                                        {estaCompletado ? (
                                            <CheckCircle2 className="h-5 w-5 text-sage-700 dark:text-dark-text-muted shrink-0" />
                                        ) : (
                                            <Circle className="h-5 w-5 text-slate-300 dark:text-dark-border shrink-0" />
                                        )}
                                        <span className={`text-xs font-medium transition-all ${estaCompletado ? 'line-through text-slate-400' : 'text-slate-700 dark:text-dark-text-base'}`}>
                                            {h.nombre}
                                        </span>
                                    </button>
                                    <button 
                                        onClick={() => eliminarHabito(h.id)}
                                        className="p-1 text-slate-300 hover:text-rose-500 rounded-md md:opacity-0 group-hover:opacity-100 transition-all cursor-pointer"
                                    >
            ...
            <div className="bg-white dark:bg-dark-card p-6 rounded-3xl border border-sage-200/60 dark:border-dark-border shadow-2xs">
                <div className="flex items-center justify-between mb-6 pb-2 border-b border-slate-100 dark:border-dark-border">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-serif font-bold text-base text-slate-800 dark:text-dark-text-base">Consistencia Mensual</h3>
                    </div>
                    <div className="flex items-center gap-4">
                        <span className="text-xs font-bold font-serif text-sage-900 dark:text-dark-sage-light capitalize">
                            {mesVisualizado.toLocaleDateString('es-AR', { month: 'long', year: 'numeric' })}
                        </span>
                ...
                <div className="grid grid-cols-7 gap-1.5 max-w-xl text-center">
                    {diasDeLaSemanaNombre.map(d => (
                        <span key={d} className="text-[10px] font-bold text-slate-400 dark:text-dark-text-muted uppercase tracking-wider py-1">{d}</span>
                    ))}
                    ...
                    {obtenerDiasDelMes(mesVisualizado).map((fechaCelda, index) => {
                        ...
                        if (programados > 0 && porcentaje === 100) {
                            estilosBoton = "bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400 font-bold";
                        }

                        return (
                            <button
                                key={fechaCelda.getTime()}
                                type="button"
                                onClick={() => setFechaSeleccionada(fechaCelda)}
                                className={`aspect-square rounded-xl flex flex-col items-center justify-center text-xs transition-all relative border cursor-pointer ${estilosBoton} ${
                                    esDiaSeleccionadoActivo 
                                        ? 'ring-2 ring-sage-600 dark:ring-dark-sage-light border-transparent font-bold scale-105 shadow-3xs' 
                                        : esFechaClave
                                        ? 'border-indigo-400 dark:border-indigo-500 bg-indigo-50/50 dark:bg-indigo-950/10 shadow-xs'
                                        : 'border-transparent'
                                }`}
                            >
                                <span className={esFechaClave ? "text-indigo-700 dark:text-indigo-400 font-bold" : ""}>
                                    {fechaCelda.getDate()}
                                </span>
                                
                                {programados > 0 && (
                                    <div 
                                        className={`w-1.5 h-1.5 rounded-full absolute bottom-1.5 transition-all ${
                                            porcentaje === 100 
                ...
                <div className="flex items-center flex-wrap gap-4 mt-6 pt-4 border-t border-slate-50 dark:border-dark-border text-[10px] font-medium text-slate-400">
                    <span className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-600" /> Pendiente</span>
                    <span className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-amber-400" /> En Progreso</span>
                    <span className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> 100% Completado</span>
                    <span className="flex items-center gap-1.5 ml-auto"><div className="w-3 h-3 rounded-md border border-indigo-400 bg-indigo-50" /> Fecha Clave</span>
                </div>
            </div>
